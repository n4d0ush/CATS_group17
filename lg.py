...
# Hyperparameter grid
rf_hp_grid = {'n_estimators': [10, 20, 50, 100, 200],
    'max_depth': [None, 10, 20],
    'min_samples_split': [2, 5, 10],
    'min_samples_leaf': [1, 2, 4],
    'max_features': [None, 5, 10, 20, 'log2', 'sqrt'],
    'criterion': ['gini', 'entropy'],
    'random_state': [42]  # Fixing random state for reproducibility
}
dt_hp_grid = {'max_depth': [2]}
...
for i, (train_index, test_index) in enumerate(outer_cv.split(X_train, y_train)):
    ...
    lr_grid_search_regul = GridSearchCV(estimator=lr_classifier, param_grid=lr_hp_grid_regularized, cv=inner_cv, scoring='accuracy',
                                  verbose=3)
    lr_grid_search_no_regul = GridSearchCV(estimator=lr_classifier, param_grid=lr_hp_grid_non_regularized, cv=inner_cv, scoring='accuracy',
                                    verbose=3)
    ...
    print("Logistic regression best hp, with regularization:", lr_best_hp_regul, "Score on validation fold, with regularization:", lr_accuracy_regul)
    print("Logistic regression best hp, no regularization:", lr_best_hp_no_regul, "Score on validation fold, no regularization:", lr_accuracy_no_regul)
    ...
    if rf_accuracy > final_score or dt_accuracy > final_score or lr_accuracy_regul > final_score or lr_accuracy_no_regul > final_score:
        if rf_accuracy >= dt_accuracy and rf_accuracy >= lr_accuracy_regul and rf_accuracy >= lr_accuracy_no_regul:
        ...
        elif dt_accuracy >= rf_accuracy and dt_accuracy >= lr_accuracy_regul and dt_accuracy >= lr_accuracy_no_regul:
            final_score = dt_accuracy
            final_hps = dt_best_hp
            final_feats = indep_features_list
            final_model = "Decision Tree"
            final_top10_feats = top_tree_importances
        elif lr_accuracy_regul >= rf_accuracy and lr_accuracy_regul >= dt_accuracy and lr_accuracy_regul >= lr_accuracy_no_regul:
            final_score = lr_accuracy_regul
            final_hps = lr_best_hp_regul
            final_feats = indep_features_list
            final_model = "Logistic Regression with regularization"
            final_top10_feats = top_logistic_importances_regul
        else:
            final_score = lr_accuracy_no_regul
            final_hps = lr_best_hp_no_regul
            final_feats = indep_features_list
            final_model = "Logistic Regression without regularization"
            final_top10_feats = top_logistic_importances_no_regul
...
print("Performance of logistic regression with regularization per fold", lr_fold_performance_lst_regul, "Average performance:",
        mean(lr_fold_performance_lst_regul))
print("Performance of logistic regression without regularization per fold", lr_fold_performance_lst_no_regul, "Average performance:",
        mean(lr_fold_performance_lst_no_regul))

print("\nSummary:")
print("Model:", final_model)
print("Score:", final_score)
print("Hyperparameters:", final_hps)
print("Features:", final_feats)
print("Top 10 features:", final_top10_feats)